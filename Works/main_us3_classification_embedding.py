...
parser.add_argument('--data', default='~/data/kidney_more_info/isangmi', help='path to dataset')
...
parser.add_argument('--result', default='../result_diagnosis_test_embedding', help='path to result')
...
def load_dataset(path):
    data = pd.read_csv(path,
                       usecols=['Diagnosis', 'KidneyLongCm', 'KidneyShortCm', 'Age', 'Sex', 'Height',
                                'Weight']).dropna()
    ...
    return dataloader, cat_dims, emb_dims
...
def train_model(train_loader, model, criterion, optimizer, epoch, print_freq):
    ...
    for i, (target, cont_input, cat_input) in enumerate(train_loader):
...
def validate_model(val_loader, model, criterion, epoch, print_freq):
    ...
    with torch.no_grad():
        ...
        for i, (target, cont_input, cat_input) in enumerate(val_loader):
...
def get_dims(path):
    data = pd.read_csv(
        path,
        usecols=['Diagnosis', 'KidneyLongCm', 'KidneyShortCm', 'Age', 'Sex', 'Height', 'Weight']).dropna()
